...
> ```
So finally we can say that we just need to find the total number of submatrices from our ```ps``` matrix that satisfies the very last condition where ```submatrix_sum(matrix[r1:r2][:c1-1]) = curr_sum - target```, which can be more formally read as ```submatrix_sum(matrix[r1:r2][:c1-1]) = (ps[r2][c2] - ps[r1-1][c2]) - target```. This is great as we have now defined everything in terms of what we could compute!

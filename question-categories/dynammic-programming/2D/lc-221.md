...
For this approach we would start the problem differenly from the start. Instead of starting at the last cell of
...
the maximal area sqaure that could be formed there. However, when we approach this probelm with the bottom up
approach we would simply reverse on the process for what was done by initalizing values on the extremes of
`dp` in terms of the squares at the top and on the very left.

This means that differing from the Top - Down approach we asking oursevles the question what is the maximal
...
## Implementation
**(Top - down) Memoization**  
In terms of implementation we would initalize a `dp` array which is 2D and matches the exact dimensions of
the input `matrix`. 
